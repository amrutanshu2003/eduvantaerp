...
const Dashboard = () => {
  ...
  const quickActions = [
    { label: "View Notices", route: "/staff/notices", icon: FiFileText, color: "bg-blue-500/10 text-blue-600 dark:text-blue-400" },
    ...(canManageLibrary(user) ? [{ label: "Manage Library", route: "/staff/library/books", icon: FiBookOpen, color: "bg-purple-500/10 text-purple-600 dark:text-purple-400" }] : []),
    ...(canManageTransport(user) ? [{ label: "Manage Transport", route: "/staff/transport/vehicles", icon: FiTruck, color: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" }] : []),
    ...(isDriverUser(user) ? [{ label: "My Route", route: "/staff/transport/my-route", icon: FiMap, color: "bg-orange-500/10 text-orange-600 dark:text-orange-400" }] : []),
    ...(canManageHostel(user) ? [{ label: "Manage Hostels", route: "/staff/hostels", icon: FiHome, color: "bg-pink-500/10 text-pink-600 dark:text-pink-400" }] : []),
    ...(isHostelSecurityUser(user) ? [{ label: "Hostels View", route: "/staff/hostels", icon: FiHome, color: "bg-cyan-500/10 text-cyan-600 dark:text-cyan-400" }] : []),
  ];
  ...
  return (
    <section className="space-y-6">
      ...
      <PageHeader
        ...
        description={`Manage your staff responsibilities with ${settings.appName}. Access library, transport, hostel, and other modules based on your designation.`}
      />
      ...
      <SectionCard>
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Common tasks for staff</p>
          </div>
        </div>
