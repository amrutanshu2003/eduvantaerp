  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid gap-4 rounded-[1.75rem] bg-white p-6 shadow-card md:grid-cols-5">
        ...
        <select value={filters.studentId} onChange={(event) => setFilters((current) => ({ ...current, studentId: event.target.value }))} className={filterClass}><option value="all">All Students</option>{supportData.students.map((student) => <option key={student._id} value={student._id}>{student.name || student.admissionNumber}</option>)}</select>
