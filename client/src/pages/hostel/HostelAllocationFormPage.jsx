    ...
    <section className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-[1.75rem] bg-white p-6 shadow-card">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {mode === "create" ? <select name="studentId" value={form.studentId} onChange={handleChange} className={inputClass}><option value="">Select Student</option>{supportData.students.map((student) => <option key={student._id} value={student._id}>{student.userId?.name || student.admissionNumber}</option>)}</select> : <input value={supportData.students.find((student) => student._id === form.studentId)?.userId?.name || "Assigned Student"} readOnly className={`${inputClass} bg-slate-50`} />}
