  ...
  return (
    <section className="space-y-6">
      <PageHeader
        ...
        actions={
          !overdueOnly && !studentHistory ? (
            <Link
              to={`${basePath}/issues/create`}
              style={{
      ...
      {!studentHistory ? (
        <div className="grid gap-4 rounded-[1.75rem] bg-white p-6 shadow-card md:grid-cols-2">
          <select
      ...
      {issues.length === 0 ? (
        <EmptyState title="No book issues found" description="Issued and returned book records will appear here." />
      ) : (
        <div className="grid gap-4">
          {issues.map((issue) => (
            <div key={issue._id} className="rounded-[1.75rem] bg-white p-6 shadow-card">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h3 className="text-xl font-semibold text-ink">{issue.bookId?.title || "Book Issue"}</h3>
                  <p className="mt-2 text-sm text-slate-600">
                    {issue.studentId?.name || issue.studentId?.userId?.name || "Student"} • Due {formatDate(issue.dueDate)}
                  </p>
                </div>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-5">
                <div>
                  ...
                  <p className="mt-2 font-semibold text-ink">{formatDate(issue.issueDate)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Return Date</p>
                  <p className="mt-2 font-semibold text-ink">{formatDate(issue.returnDate)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Fine</p>
                  <p className="mt-2 font-semibold text-ink">{formatCurrency(issue.fineAmount)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Issued By</p>
                  <p className="mt-2 font-semibold text-ink">{issue.issuedBy?.name || "Not available"}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Remarks</p>
                  <p className="mt-2 font-semibold text-ink">{issue.remarks?.trim() || "No remarks"}</p>
                </div>
              </div>

              <div className="mt-5 flex flex-wrap gap-2">
                {!issue.returnDate && issue.status !== "lost" ? (
                  <button
                    type="button"
                ...
                <button
                  type="button"
                  onClick={() => handleFine(issue._id, issue.fineAmount)}
                  className="rounded-full border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700"
                >
                  Update Fine
                </button>
                {issue.studentId?._id && basePath === "/admin/library" ? (
                  <Link
                    to={`/admin/library/students/${issue.studentId._id}/history`}
                    className="rounded-full border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700"
                  >
                    Student History
                  </Link>
