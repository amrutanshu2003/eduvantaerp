            ...
            <div key={assignment._id} className="rounded-[1.75rem] bg-white p-6 shadow-card">
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-4">
                <div>
                  ...
                  <p className="mt-2 font-semibold text-ink">{formatLabel(assignment.assignmentType)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Due Date</p>
                  <p className="mt-2 font-semibold text-ink">{formatDate(assignment.dueDate)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Academic Group</p>
                  <p className="mt-2 font-semibold text-ink">{assignment.academicGroupId?.className || [assignment.academicGroupId?.department, assignment.academicGroupId?.course].filter(Boolean).join(" - ") || "-"}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Max Marks</p>
                  <p className="mt-2 font-semibold text-ink">{assignment.maxMarks || "-"}</p>
                </div>
              </div>
            </div>
          ))}
