...
const InstituteSettings = () => {
  ...
  const fetchAllSettings = async () => {
    try {
      const [erpRes, labelRes, moduleRes, academicRes] = await Promise.all([
        api.get("/settings/institute/erp"),
        api.get("/settings/institute/labels"),
        api.get("/settings/institute/modules"),
        api.get("/settings/institute/academic"),
      ]);
  ...
  const fetchFormSettings = async (entity) => {
    try {
      const { data } = await api.get(`/settings/institute/forms/${entity}`);
  ...
  const handleSaveERPSettings = async () => {
    ...
    setMessage("");
    try {
      const { data } = await api.put("/settings/institute/erp", erpSettings);
      setErpSettings(data.erpSettings);
      setMessageTone("success");
      setMessage("Institute ERP settings updated successfully");
    } catch (error) {
  ...
  const handleSaveLabelSettings = async () => {
    ...
    setMessage("");
    try {
      const { data } = await api.put("/settings/institute/labels", labelSettings);
      setLabelSettings(data.labelSettings);
      setMessageTone("success");
      setMessage("Institute label settings updated successfully");
    } catch (error) {
      setMessageTone("error");
  ...
  const handleSaveModuleSettings = async () => {
    ...
    setMessage("");
    try {
      const { data } = await api.put("/settings/institute/modules", moduleSettings);
      setModuleSettings(data.moduleSettings);
      setMessageTone("success");
      setMessage("Institute module settings updated successfully");
    } catch (error) {
      setMessageTone("error");
  ...
  const handleResetToGlobal = async (type) => {
    if (!window.confirm("Are you sure you want to reset to global settings?")) return;

    setSubmitting(true);
    setMessage("");
    try {
      if (type === "erp") {
        await api.post("/settings/institute/erp/reset");
        await fetchAllSettings();
      } else if (type === "labels") {
        await api.post("/settings/institute/labels/reset");
        await fetchAllSettings();
      } else if (type === "modules") {
        await api.post("/settings/institute/modules/reset");
        await fetchAllSettings();
      }
      setMessageTone("success");
      setMessage(`${type.charAt(0).toUpperCase() + type.slice(1)} settings reset to global defaults successfully`);
    } catch (error) {
      setMessageTone("error");
  ...
  const handleSaveAcademicSettings = async () => {
    ...
    setMessage("");
    try {
      const { data } = await api.put("/settings/institute/academic", academicSettings);
      setAcademicSettings(data.academicSettings);
      setMessageTone("success");
      setMessage("Institute academic settings updated successfully");
    } catch (error) {
      setMessageTone("error");
  ...
  const handleSaveFormSettings = async () => {
    ...
    setMessage("");
    try {
      const { data } = await api.put(`/settings/institute/forms/${selectedFormEntity}`, formSettings);
      setFormSettings(data.formSettings);
      setMessageTone("success");
      setMessage(`Institute form settings for ${selectedFormEntity} updated successfully`);
    } catch (error) {
      setMessageTone("error");
  ...
  const handleApplyTemplate = async (template) => {
    ...
    setMessage("");
    try {
      await api.post("/settings/institute/academic/reset", { template });
      await fetchAllSettings();
      setMessageTone("success");
  ...
  return (
    <section className="space-y-6">
      <PageHeader
        ...
        description="Customize branding, labels, and modules for your institute."
      />
      ...
      {activeTab === "branding" && (
        <form onSubmit={(e) => { e.preventDefault(); handleSaveERPSettings(); }} className="rounded-[1.75rem] bg-white p-6 shadow-card dark:bg-slate-900">
          <div className="mb-6 flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => handleResetToGlobal("erp")}
              disabled={submitting}
      ...
      {activeTab === "labels" && (
        <form onSubmit={(e) => { e.preventDefault(); handleSaveLabelSettings(); }} className="rounded-[1.75rem] bg-white p-6 shadow-card dark:bg-slate-900">
          <div className="mb-6 flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => handleResetToGlobal("labels")}
              disabled={submitting}
              className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
            >
              Reset to Global
      ...
      {activeTab === "modules" && (
        <form onSubmit={(e) => { e.preventDefault(); handleSaveModuleSettings(); }} className="rounded-[1.75rem] bg-white p-6 shadow-card dark:bg-slate-900">
          <div className="mb-6 flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => handleResetToGlobal("modules")}
              disabled={submitting}
              className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
            >
              Reset to Global
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSaveAcademicSettings(); }} className="rounded-[1.75rem] bg-white p-6 shadow-card dark:bg-slate-900">
          <div className="mb-6 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => handleResetToGlobal("academic")}
                disabled={submitting}
                className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
              >
                Reset to Global
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSaveFormSettings(); }} className="rounded-[1.75rem] bg-white p-6 shadow-card dark:bg-slate-900">
          <div className="mb-6 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => handleResetToGlobal("forms")}
                disabled={submitting}
                className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
              >
                Reset to Global
