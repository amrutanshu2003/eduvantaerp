...
const formDefaults = {
  ...
  password: "",
  status: "active",
  permissions: "students.view,students.create,students.update,ui.customize",
};
...
const CreateInstituteAdmin = () => {
  ...
  useEffect(() => {
    const fetchInstitute = async () => {
      try {
        ...
        setInstituteName(data.institute.name);
      } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await api.post(`/institutes/${id}/admin`, {
        ...formData,
      ...
      window.alert("Institute admin created successfully");
      navigate(`/super-admin/institutes/${id}`);
    } catch (error) {
      setErrorMessage(error.response?.data?.message || "Unable to create institute admin");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return <LoadingBlock message="Loading institute for admin creation..." />;
  }

  return (
    <section className="space-y-6">
      <div className="rounded-[1.75rem] bg-white p-6 shadow-card">
        <h1 className="text-3xl font-semibold text-ink">Create Institute Admin</h1>
        <p className="mt-3 text-sm leading-6 text-slate-600">
          Create an admin account for <span className="font-semibold">{instituteName}</span>. This user will belong only to that institute.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-[1.75rem] bg-white p-6 shadow-card">
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <div>
            ...
            <input name="phone" value={formData.phone} onChange={handleChange} className={inputClassName} />
          </div>
          <div>
        ...
        <div className="mt-6 space-y-4">
          ...
          >
            {submitting ? "Creating..." : "Create Institute Admin"}
          </button>
        </div>
      </form>
    </section>
