...
const DashboardLayout = () => {
  ...
  return (
    <div ref={shellRef} className="h-screen w-full overflow-hidden max-w-full" style={{ background: "var(--theme-app-bg)" }}>
      ...
      <div className="flex h-screen w-full overflow-hidden max-w-full">
        {/* Desktop Sidebar */}
        <aside className="sticky top-0 hidden h-full w-72 shrink-0 overflow-y-auto overflow-x-hidden no-scrollbar md:block">
          <Sidebar />
        </aside>
        ...
        {mobileSidebarOpen && (
          <div className="fixed inset-0 z-[260] md:hidden">
            ...
            />
            {/* Drawer */}
            <div className="absolute inset-y-0 left-0 z-[270] flex w-72 max-w-[86vw] flex-col shadow-2xl bg-white dark:bg-slate-900 animate-in slide-in-from-left duration-300 ease-out">
              <div className="flex items-center justify-between px-4 pt-4 pb-2 border-b border-slate-200 dark:border-slate-700">
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">Menu</span>
                <button
                  type="button"
                  onClick={() => setMobileSidebarOpen(false)}
                  className="flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
